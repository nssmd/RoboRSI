...
from roborsi.embodied.embodiment.hardware.discovery import HardwareDiscovery
from roborsi.embodied.embodiment.hardware.scan import (
    _list_serial_ports,
    scan_serial_ports,
...
def test_list_serial_ports_uses_pyserial_devices_on_windows() -> None:
    ...
    ), patch("roborsi.embodied.embodiment.hardware.scan.os.name", "nt"):
...
def test_list_serial_ports_matches_lerobot_range_on_linux() -> None:
    ...
    ), patch("roborsi.embodied.embodiment.hardware.scan.os.name", "posix"), patch(
        "roborsi.embodied.embodiment.hardware.scan.sys.platform", "linux",
    ):
        ports = _list_serial_ports()
...
def test_scan_serial_ports_merges_port_list_with_linux_symlink_aliases() -> None:
    with (
        patch("roborsi.embodied.embodiment.hardware.scan._list_serial_ports", return_value=["/dev/ttyACM0"]),
        patch(
            "roborsi.embodied.embodiment.hardware.scan._read_symlink_map",
            side_effect=[
                {"/dev/ttyACM0": "/dev/serial/by-path/pci-0:2.1"},
                {"/dev/ttyACM0": "/dev/serial/by-id/usb-ABC-if00"},
            ],
        ),
        patch("roborsi.embodied.embodiment.hardware.scan.os.path.exists", return_value=True),
    ):
...
def test_scan_serial_ports_uses_lerobot_compatible_range() -> None:
    with (
        patch("roborsi.embodied.embodiment.hardware.scan._read_symlink_map", return_value={}),
        patch("roborsi.embodied.embodiment.hardware.scan.os.path.exists", return_value=True),
        patch("roborsi.embodied.embodiment.hardware.scan._list_serial_ports", return_value=["/dev/cu.usbmodemA"]),
    ):
        ports = scan_serial_ports()
...
def test_serial_patterns_for_platform_macos_uses_cu_only() -> None:
    with patch("roborsi.embodied.embodiment.hardware.scan.sys.platform", "darwin"):
...
def test_serial_patterns_for_platform_linux_uses_tty() -> None:
    with patch("roborsi.embodied.embodiment.hardware.scan.sys.platform", "linux"):
        patterns = serial_patterns_for_platform()
...
def test_scan_serial_ports_macos_only_returns_cu_devices() -> None:
    with (
        patch(
            "roborsi.embodied.embodiment.hardware.scan._list_serial_ports",
