...
from roborsi.agent.tools.web import WebSearchTool
from roborsi.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    async def mock_get(self, url, **kw):
        ...
        return _response(json={
            "web": {"results": [{"title": "RoboRSI", "url": "https://example.com", "description": "AI assistant"}]}
        })

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="brave-key")
    result = await tool.execute(query="roborsi", count=1)
    assert "RoboRSI" in result
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("roborsi.agent.tools.web.DDGS", MockDDGS, raising=False)
    import roborsi.agent.tools.web as web_mod
