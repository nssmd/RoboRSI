...
metadata:
  ...
  base_tools: [look, find_pixel, unproject_pixel, grasp_object, place_object_in, place_on_surface, move_to_pose, gripper]
params:
...
`place_object_in` for receptacles or `place_on_surface` for exposed supports)
against a LIBERO task. Final success is recorded only after the episode by the
harness.
