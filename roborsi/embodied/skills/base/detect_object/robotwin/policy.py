"""base.robotwin.detect_object — Grounded-DINO + SAM object grounding.
...
`detect()` is a PURE function and MUST NOT import robotwin_agent at module
or call scope — keep model loading free of any sim dependency to avoid a
circular import (robotwin_tools imports detect from here). Only
`dispatch_runtime` touches rollout, via a lazy in-function import.
"""
...
# Long-horizon executor and many composite skills call find_pixel /
...
def dispatch_runtime(state, args: dict[str, Any]):
    ...
    dets = detect(np.asarray(head), obj, top_k=top_k,
    ...
    out_list = [{"bbox": list(d.bbox), "centroid": list(d.centroid),
                 "score": round(d.score, 3)} for d in dets]
    return ({"ok": True, "detections": out_list, "best": out_list[0],
             "n": len(out_list)}, obs)
