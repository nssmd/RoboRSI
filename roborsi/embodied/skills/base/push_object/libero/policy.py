...
_MAX_CONTACT_BACKOFF = 0.07
_CONTACT_EDGE_PADDING = 0.003
...
def _visual_contact_backoff(
    ...
    back_extent = max(0.0, -float(np.percentile(projected, 5)))
    return float(
        np.clip(
...
def dispatch_runtime(state: Any, args: dict[str, Any]):
    ...
    max_distance = _bounded(args.get("max_distance"), 0.20, 0.04, 0.25)
    ...
    contact_z_offset = _bounded(
        args.get("contact_z_offset"), 0.015, -0.02, 0.05
    )
    ...
    hover = contact - xy_direction * effective_standoff
    hover[2] += hover_clearance
    push_target = contact + xy_direction * push_distance
    _, current_quat, _ = control.read_pose()
    quat = np.asarray(current_quat, dtype=float)
    orientation_mode = "preserve_current"
    ...
    hover_within = bool(
        reached and float(np.linalg.norm(measured - hover)) <= 0.05
    )
    if not hover_within:
        ...
        if hover_within:
            orientation_mode = "canonical_topdown_fallback"
    if not hover_within:
        return _failure(
            env,
            "could not reach the planar-push hover pose",
