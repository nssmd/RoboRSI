...
from roborsi.embodied.skills.base._lib.libero.pyroki_protocol import (
    WIRE_PROTOCOL as _PYROKI_WIRE_PROTOCOL,
)
            ...
            if trajectory is not None:
                ...
                for waypoint in trajectory[1:]:
                    for _ in range(
                        ...
                        delta = np.asarray(waypoint, dtype=float) - self._arm_qpos()
                        if float(np.max(np.abs(delta))) <= _JOINT_RECOVERY_WAYPOINT_TOL:
                            break
