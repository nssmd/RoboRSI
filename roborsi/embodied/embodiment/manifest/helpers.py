...
def get_roborsi_home(home: str | Path | None = None) -> Path:
    """Return RoboRSI home directory, honoring ROBORSI_HOME env var."""
    if home is not None:
        return Path(home).expanduser()
    return Path(os.environ.get("ROBORSI_HOME", "~/.roborsi")).expanduser()


def get_manifest_path(home: Path | None = None) -> Path:
    """Return the manifest.json path under *home*."""
    return (home or get_roborsi_home()) / "workspace" / "embodied" / "manifest.json"



def get_calibration_root(home: Path | None = None) -> Path:
    """Return the calibration directory under *home*."""
    return (home or get_roborsi_home()) / "workspace" / "embodied" / "calibration"
...
def _default_manifest(home: Path | None = None) -> dict[str, Any]:
    ...
    base = (home or get_roborsi_home()) / "workspace" / "embodied"
    return {
        "version": 2,
        "arms": [],
        "hands": [],
...
def _resolve_serial_interface(port: str) -> "SerialInterface":
    ...
    from roborsi.embodied.embodiment.hardware.scan import scan_serial_ports
    from roborsi.embodied.embodiment.interface.serial import SerialInterface
...
def _validate_cameras(cameras: Any) -> None:
    ...
    for cam in cameras:
        ...
        from roborsi.embodied.embodiment.manifest.binding import validate_camera_side
...
def _probe_hand_slave_id(hand_type: str, port: str) -> int:
    ...
    from roborsi.embodied.embodiment.hand.modbus import probe_modbus_slave_ids
...
def set_camera(
    ...
    from roborsi.embodied.embodiment.hardware.scan import scan_cameras
