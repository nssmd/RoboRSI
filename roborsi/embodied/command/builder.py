...
def _wrapper_args(action: str) -> list[str]:
    ...
    return [sys.executable, "-m", "roborsi.embodied.command.wrapper", action]
...
def _bimanual_args(
    ...
    """
    from roborsi.embodied.embodiment.manifest.helpers import ensure_bimanual_cal_dir
    ...
    if cameras:
        ...
        if unsided:
            from roborsi.embodied.command.helpers import ActionError
            raise ActionError(
