...
def dataset_path(manifest: Any, name: str, fallback: Path | None = None) -> Path:
    ...
    return get_roborsi_home() / "workspace" / "embodied" / "datasets" / "local" / name
...
def policy_path(manifest: Any, name: str) -> Path:
    ...
    from roborsi.embodied.embodiment.manifest.helpers import get_roborsi_home
    return get_roborsi_home() / "workspace" / "embodied" / "policies" / name


def logs_dir() -> Path:
    """Return the embodied jobs log directory."""
    from roborsi.embodied.embodiment.manifest.helpers import get_roborsi_home
    return get_roborsi_home() / "workspace" / "embodied" / "jobs"
