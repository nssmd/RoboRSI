  ...
  vlm_prompts:
    instruction: |
      ...
      0. When `visual_pick_place` is listed in the available tools, make it the
         FIRST CHOICE for one movable object going to an exposed surface. Pass
         the exact source and target wording plus placement=surface. Containers
         remain on the individual grasp_object + place_object_in path. Inspect
