"""Shared driver for LIBERO task-family executors.

The caller supplies a Task Family and benchmark task key. The executor loads
the family guidance, resets the selected task, and runs the shared tool loop.
Final success is assigned by the backend after execution and is never included
in the agent-visible context. The same executor supports short and long-horizon
LIBERO task profiles.
"""
