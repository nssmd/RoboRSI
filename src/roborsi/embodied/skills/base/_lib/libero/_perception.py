...
def vlm_point(state, obj: str, location: str = ""):
    ...
    model = os.environ.get("ROBORSI_PERCEPTION_MODEL", "responses/gpt-5.6-sol")
...
def _ranked_localization_candidates(rgb, obj: str, pointer_uv=None):
    ...
    try:
        from roborsi.embodied.skills.base._lib.libero.detector import (
            detect,
        )
...
def grasps_at_pixel(env, u: int, v: int, top_k: int = 3):
    ...
    if port <= 0:
        ...
        try:
            from roborsi.embodied.sim.libero.graspgen_client import _grasps_from_cloud
