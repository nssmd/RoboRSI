...
def detect(
    ...
    *,
    ...
    if image_rgb.dtype != np.uint8:
        image_rgb = (
            (image_rgb * 255).clip(0, 255).astype(np.uint8)
            if image_rgb.max() <= 1
    ...
    sam_in = sp(images=pil, input_boxes=[boxes.tolist()], return_tensors="pt").to(_DEVICE)
    sam_out = sm(**sam_in)
...
def dispatch_runtime(state, args: dict[str, Any]):
    ...
    if not obj:
        return (
            {"ok": False, "reason": "object name required"},
    ...
    dets = detect(head_arr, obj, top_k=top_k, box_threshold=box_thr, text_threshold=text_thr)
    if not dets:
        return (
            {
                "ok": False,
                "reason": f"no detections for '{obj}'",
    ...
    for d in dets:
        x0, y0, x1, y1 = d.bbox
        frac = (max(0, x1 - x0) * max(0, y1 - y0)) / frame_area if frame_area else 0.0
        entry = {
            "bbox": list(d.bbox),
            "centroid": list(d.centroid),
            "score": round(d.score, 3),
            "bbox_frame_frac": round(frac, 3),
        }
        if frac > whole_frame_frac:
            entry["whole_frame"] = True
            entry["note"] = (
                f"bbox covers {frac:.0%} of the frame — this is the "
    ...
    result = {"ok": True, "detections": out_list, "best": out_list[0], "n": len(out_list)}
    if out_list[0].get("whole_frame"):
        result["all_whole_frame"] = True
        result["hint"] = (
            f"EVERY detection for '{obj}' fills the frame — it was "
