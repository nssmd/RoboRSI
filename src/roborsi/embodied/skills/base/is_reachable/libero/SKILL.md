...
category: control
version: 0.2.0
description: Reject obviously unreachable finite target coordinates with a conservative workspace-envelope
  heuristic before a full arm motion.
args:
  x:
    type: float
    description: World-frame target X; provide x/y/z or pos.
  y:
    type: float
    description: World-frame target Y.
  z:
    type: float
    description: World-frame target Z.
  pos:
    type: list
    description: Finite target [x,y,z]; alternative to x/y/z.
returns:
  ok: bool
  reachable: bool
