...
category: perception
version: 0.1.0
description: Render fresh calibrated orbit RGB-D views of the current LIBERO scene and attach either one
  view or a labeled contact sheet.
args:
  view:
    type: string
    description: Optional named orbit view. Omit for a labeled contact sheet.
  image_size:
    type: int
    default: 512
    description: Per-view resolution, clamped to 256-512.
returns:
  ok: bool
  views: list
  image_size: int
...
On-demand calibrated multi-view observation from the current rendered scene.
