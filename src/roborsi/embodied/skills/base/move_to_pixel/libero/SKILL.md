...
category: control
version: 0.1.0
description: Move the end-effector above a point you clicked in a camera image — unprojects the pixel
  to a world XYZ, then servos there (with a vertical approach offset).
args:
  u:
    type: int
    required: true
  ...
  approach_z:
    type: float
    description: Extra height to stop above the unprojected point (m, default 0.05).
  gripper:
    type: string
    enum:
    - open
